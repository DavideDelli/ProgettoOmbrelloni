...
<div class="container">
    ...
    <nav>
        <a href="../../index.php">Home</a> <a href="../../mappa.php">Mappa Spiaggia</a> <a href="../../le_mie_prenotazioni.php" class="active">Le mie Prenotazioni</a> <a href="../../profilo.php">Il mio profilo (<?= htmlspecialchars($_SESSION['nome_cliente']) ?>)</a>
    </nav>
    <main>
        <?php if ($errore): ?>
            <div class="messaggio errore glass-panel">
                ...
                <a href="../../le_mie_prenotazioni.php" class="button" style="margin-top: 1rem;">Torna alle mie prenotazioni</a>
            </div>
        <?php endif; ?>

        <?php if ($prenotazione && !$errore): ?>
            <form action="processa_modifica.php" method="POST" class="form-prenotazione glass-panel">
                ...
                <fieldset style="border: 1px solid rgba(255,255,255,0.2); background: transparent;">
                    <legend style="color: #FFF;">Modifica Periodo e Pacchetto</legend>
                    <div class="form-group">
                        <label for="data_nuova"><?= ($tipo_prenotazione === 'settimanale') ? 'Nuova data di inizio (la fine verrà ricalcolata):' : 'Nuova data:' ?></label>
                        <input type="date" id="data_nuova" name="data_nuova" value="<?= htmlspecialchars($prenotazione['data_inizio']) ?>" required>
                    </div>
                    <div class="form-group">
                        <label>Nuovo pacchetto:</label>
            ...
            <script>
                ...
                function aggiornaPrezzo() { 
                    const scelta = document.querySelector('input[name="cod_tariffa_nuovo"]:checked'); 
                    if (scelta) { 
                        const nuovoPrezzo = parseFloat(scelta.dataset.prezzo); 
                        prezzoTotaleEl.innerHTML = `<strong>Nuovo Prezzo:</strong> €${nuovoPrezzo.toFixed(2).replace('.', ',')}`;
                    } 
                }
